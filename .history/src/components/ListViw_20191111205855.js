...
class SectionList extends React.Component{
    ...
    filterPlanByKey(keyArray,value){
   //console.log("saqui ",this.state.results,key)

      var results=this.state.results;
      var internalArray=[];

      for( let item in results){  //4

        //results[0]  = {messages:10,internet:20,name:"camilo"}
        //results[1]  = {messages:10,internet:20,name:"camilo"}
        ...
        for(let key in results[item]){
         ...
         //menssages == messages
         let valueProperty=results[item][key];
         let keyProperty=results[item];
          if(key==keyArray){
              if(valueProperty <=value){
                internalArray.push(keyProperty);
              }
          //  console.log(key,results[item][key]);
          }
